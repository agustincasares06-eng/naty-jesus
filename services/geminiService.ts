...
export const getAIResponse = async (userPrompt: string, history: {role: string, content: string}[]): Promise<AIResponse> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "Eres el Concierge Virtual de la boda de Natalia y Jesús. Tu tono es extremadamente educado, cálido y servicial. Ayuda a los invitados con detalles de la boda. El Civil es el viernes 20 a las 11:30 am. El sábado 21 la ceremonia es a las 18:00 en la Catedral y la fiesta es en 'Casa de Piedra' a las 19:30. El dress code es 'Elegante Sport'. Si preguntan por regalos, menciona el alias 'artesanales.antojito' para apoyar su emprendimiento. Siempre responde en español.",
        tools: [{ googleSearch: {} }]
      }
    });
